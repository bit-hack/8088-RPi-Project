...
static uint32_t gpioLookup(uint32_t wpi) {
    switch (wpi) {
    case 8:  return 2;
    case 9:  return 3;
    case 7:  return 4;
...
void gpioLevelSet(uint32_t pin, int level) {
    ...
    uint32_t gpio = gpioLookup(pin);
    bcm2835_set_level(chip, gpio, (level == 0) ? GPIO_LEVEL_LOW : GPIO_LEVEL_HIGH);
}

gpioLevel gpioLevelGet(uint32_t pin) {
    
    uint32_t gpio = gpioLookup(pin);
    return bcm2835_get_level(chip, gpio) ? GPIO_LEVEL_HIGH : GPIO_LEVEL_LOW;
}

void gpioDirSet(uint32_t pin, gpioDir dir) {
    
    uint32_t gpio = gpioLookup(pin);
    bcm2835_set_fsel(chip, gpio, (dir == GPIO_DIR_IN) ? FUNC_IP : FUNC_OP);
}
