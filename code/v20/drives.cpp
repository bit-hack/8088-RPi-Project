...
void Get_Disk_Parameters_A() {
  ...
  // Start at port 11
  Write_Memory_Byte(
      0xF800B, Media_Descriptor[Floppy[0x15] & 0x0F]); // Media Descriptor - BL

  Write_Memory_Byte(0xF800C,
                    Floppy[0x1A]); // Low byte heads per cylinder - DH -1
...
void Get_Disk_Parameters_C() {
  ...
  if (drive[0x1BE] == 0x80) {
    Hidden_Sectors = drive[0x1C6] + (drive[0x1C7] << 8) + (drive[0x1C8] << 16) +
                     (drive[0x1C9] << 24);
  } else if (drive[0x1CE] == 0x80) {
    Hidden_Sectors = drive[0x1D6] + (drive[0x1D7] << 8) + (drive[0x1D8] << 16) +
                     (drive[0x1D9] << 24);
  } else if (drive[0x1DE] == 0x80) {
    Hidden_Sectors = drive[0x1E6] + (drive[0x1E7] << 8) + (drive[0x1E8] << 16) +
                     (drive[0x1E9] << 24);
  } else if (drive[0x1EE] == 0x80) {
    Hidden_Sectors = drive[0x1F6] + (drive[0x1F7] << 8) + (drive[0x1F8] << 16) +
                     (drive[0x1F9] << 24);
  }
  // No boot partion MBR
  else {
  ...
  // char Media_Descriptor[0x10] = {0x04, 0, 0, 0, 0, 0, 0, 0, 0x00, 0x02, 0, 0,
  // 0, 0x01, 0, 0}; Start at port 11 Write_Memory_Byte(0xF800B,
  // Media_Descriptor[drive[0x15] & 0x0F]);    //Media Descriptor - BL

  Write_Memory_Byte(0xF800C, drive[0x1A]); // Low byte heads per cylinder - DH
                                           // -1
...
void Int13() {
  ...
  if (Int13_Command != 0xFF) {
    ...
    {
      if (int13_data[6] == 0x00) {
        Get_Disk_Parameters_A();
      }
      if (int13_data[6] == 0x80) {
        Get_Disk_Parameters_C();
      }
    }
