...
void Print_Char_40x25(SDL_Renderer *Renderer, int x, int y,
...
void Text_Mode_40x25(SDL_Renderer *Renderer, char *Video_Memory,
  ...
  for (int j = 0; j < 25; j++) {
    for (int i = 0; i < 40; i++) {
      Print_Char_40x25(Renderer, 2 + (i * 16), 2 + (j * 16),
                       Video_Memory[index]);
      index++;
      index++;
    }
  }
  Print_Char_40x25(Renderer, (Cursor_Position[0] * 16) + 2,
                   (Cursor_Position[1] * 16) + 2, 0xDB);
  SDL_RenderPresent(Renderer);
}

void Print_Char_80x25(SDL_Renderer *Renderer, int x, int y,
                      unsigned char Ascii_value) {
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      if (((_8x8char[Ascii_value][j] >> i) & 1) == 0) {
...
void Text_Mode_80x25(SDL_Renderer *Renderer, char *Video_Memory,
  ...
  for (int j = 0; j < 25; j++) {
    for (int i = 0; i < 80; i++) {
      Print_Char_80x25(Renderer, 2 + (i * 8), 2 + (j * 16),
                       Video_Memory[index]);
      index++;
      index++;
    }
  }
  Print_Char_80x25(Renderer, (Cursor_Position[0] * 8) + 2,
                   (Cursor_Position[1] * 16) + 2, 0xDB);
  SDL_RenderPresent(Renderer);
}

void print_pixs_mode4_palette_0(SDL_Renderer *Renderer, int x, int y,
                                unsigned char Ascii_value) {
  for (int i = 0; i < 4; i++) {
...
void Graphics_Mode_320_200_Palette_0(SDL_Renderer *Renderer,
  ...
  for (int j = 0; j < 100; j++) {
    for (int i = 0; i < 80; i++) {
      print_pixs_mode4_palette_0(Renderer, 2 + (i * 8), 2 + (j * 4),
                                 Video_Memory[index]);
      index++;
    }
  }
  ...
  for (int j = 0; j < 100; j++) {
    for (int i = 0; i < 80; i++) {
      print_pixs_mode4_palette_0(Renderer, 2 + (i * 8), 4 + (j * 4),
                                 Video_Memory[index]);
      index++;
    }
  }
...
void print_pixs_mode4_palette_1(SDL_Renderer *Renderer, int x, int y,
                                unsigned char Ascii_value) {
  for (int i = 0; i < 4; i++) {
    if (((Ascii_value >> (i + i)) & 3) == 0) {
      SDL_SetRenderDrawColor(Renderer, 0, 0, 0, SDL_ALPHA_OPAQUE);
...
void Graphics_Mode_320_200_Palette_1(SDL_Renderer *Renderer,
  ...
  for (int j = 0; j < 100; j++) {
    for (int i = 0; i < 80; i++) {
      print_pixs_mode4_palette_1(Renderer, 2 + (i * 8), 2 + (j * 4),
                                 Video_Memory[index]);
      index++;
    }
  }
  ...
  for (int j = 0; j < 100; j++) {
    for (int i = 0; i < 80; i++) {
      print_pixs_mode4_palette_1(Renderer, 2 + (i * 8), 4 + (j * 4),
                                 Video_Memory[index]);
      index++;
    }
  }
