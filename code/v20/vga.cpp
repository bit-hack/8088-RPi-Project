...
void Print_Char_9x16(SDL_Renderer *Renderer, int x, int y, uint8_t Ascii_value) {
  for (int i = 0; i < 9; i++) {
    for (int j = 0; j < 16; j++) {
      if (((font_9x16[Ascii_value][j] >> (15 - i)) & 1) == 0) {
        SDL_SetRenderDrawColor(Renderer, 0, 0, 0, SDL_ALPHA_OPAQUE);
...
void print_pixs_mode4_palette_0(SDL_Renderer *Renderer, int x, int y, uint8_t Ascii_value) {
  for (int i = 0; i < 4; i++) {
...
void print_pixs_mode4_palette_1(SDL_Renderer *Renderer, int x, int y, uint8_t Ascii_value) {
