...
void Print_Char_18x16(SDL_Renderer *Renderer, int x, int y,
...
void Mode_0_40x25(SDL_Renderer *Renderer, char *Video_Memory,
  ...
  Print_Char_18x16(Renderer, (Cursor_Position[0] * 18),
                   (Cursor_Position[1] * 16), 0xDB);
  SDL_RenderPresent(Renderer);
}

void Print_Char_9x16(SDL_Renderer *Renderer, int x, int y,
                     unsigned char Ascii_value) {
  for (int i = 0; i < 9; i++) {
    for (int j = 0; j < 16; j++) {
      if (((font_9x16[Ascii_value][j] >> (15 - i)) & 1) == 0) {
...
void Mode_2_80x25(SDL_Renderer *Renderer, char *Video_Memory,
                  char *Cursor_Position) {
  int index = 0;
  for (int j = 0; j < 25; j++) {
    for (int i = 0; i < 80; i++) {
...
void Graphics_Mode_320_200_Palette_0(SDL_Renderer *Renderer,
  ...
  for (int j = 0; j < 100; j++) {
    for (int i = 0; i < 80; i++) {
      print_pixs_mode4_palette_0(Renderer, 40 + (i * 8), (j * 4),
                                 Video_Memory[index]);
      index++;
    }
  }
  ...
  for (int j = 0; j < 100; j++) {
    for (int i = 0; i < 80; i++) {
      print_pixs_mode4_palette_0(Renderer, 40 + (i * 8), 2 + (j * 4),
                                 Video_Memory[index]);
      index++;
    }
  }
...
void Graphics_Mode_320_200_Palette_1(SDL_Renderer *Renderer,
  ...
  for (int j = 0; j < 100; j++) {
    for (int i = 0; i < 80; i++) {
      print_pixs_mode4_palette_1(Renderer, 40 + (i * 8), (j * 4),
                                 Video_Memory[index]);
      index++;
    }
  }
  ...
  for (int j = 0; j < 100; j++) {
    for (int i = 0; i < 80; i++) {
      print_pixs_mode4_palette_1(Renderer, 40 + (i * 8), 2 + (j * 4),
                                 Video_Memory[index]);
      index++;
    }
  }
